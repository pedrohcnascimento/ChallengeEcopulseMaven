...
public class AvatarService {
    ...
    public Avatar create(Avatar avatar){
        if(avatar==null||avatar.getUserId()==null)throw new IllegalArgumentException("Usuário do avatar é obrigatório");
        if(userDao.findById(avatar.getUserId()).isEmpty())throw new IllegalArgumentException("Usuário não encontrado: "+avatar.getUserId()+". Cadastre o usuário antes de criar o avatar.");
        if(avatar.getName()==null||avatar.getName().isBlank())throw new IllegalArgumentException("Nome do avatar é obrigatório");
