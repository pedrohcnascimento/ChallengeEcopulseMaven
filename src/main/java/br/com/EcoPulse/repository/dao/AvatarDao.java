...
public class AvatarDao {
    ...
    public Avatar create(Avatar avatar) {
        ...
        try (Connection c=ConnectionFactory.getConnection(); PreparedStatement ps=c.prepareStatement(sql)) {
            ...
            return avatar;
        } catch(SQLException e){throw new PersistenceException("Erro ao criar avatar",e);}
    }
    public Optional<Avatar> findById(Long id){try(Connection c=ConnectionFactory.getConnection();PreparedStatement ps=c.prepareStatement("SELECT "+COLUMNS+" FROM T_CHLNG_AVATARS WHERE id=?")){ps.setLong(1,id);try(ResultSet rs=ps.executeQuery()){return rs.next()?Optional.of(map(rs)):Optional.empty();}}catch(SQLException e){throw new PersistenceException("Erro ao buscar avatar",e);}}
    public List<Avatar> findAll(){List<Avatar> result=new ArrayList<>();try(Connection c=ConnectionFactory.getConnection();PreparedStatement ps=c.prepareStatement("SELECT "+COLUMNS+" FROM T_CHLNG_AVATARS ORDER BY id");ResultSet rs=ps.executeQuery()){while(rs.next())result.add(map(rs));return result;}catch(SQLException e){throw new PersistenceException("Erro ao listar avatares",e);}}
    public Avatar update(Avatar a){String sql="UPDATE T_CHLNG_AVATARS SET name=?,visual_config=?,personality_type=?,level_number=?,experience_points=?,last_interaction=?,updated_at=? WHERE id=?";try(Connection c=ConnectionFactory.getConnection();PreparedStatement ps=c.prepareStatement(sql)){ps.setString(1,a.getName());ps.setString(2,a.getVisualConfig());ps.setString(3,a.getPersonalityType());ps.setInt(4,a.getLevel());ps.setLong(5,a.getExperiencePoints());ps.setTimestamp(6,Timestamp.from(a.getLastInteraction()));ps.setTimestamp(7,Timestamp.from(a.getUpdatedAt()));ps.setLong(8,a.getId());if(ps.executeUpdate()==0)throw new IllegalArgumentException("Avatar não encontrado: "+a.getId());return a;}catch(SQLException e){throw new PersistenceException("Erro ao atualizar avatar",e);}}
    public boolean deleteById(Long id){try(Connection c=ConnectionFactory.getConnection();PreparedStatement ps=c.prepareStatement("DELETE FROM T_CHLNG_AVATARS WHERE id=?")){ps.setLong(1,id);return ps.executeUpdate()>0;}catch(SQLException e){throw new PersistenceException("Erro ao excluir avatar",e);}}
